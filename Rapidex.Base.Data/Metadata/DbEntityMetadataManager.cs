...
namespace Rapidex.Data.Metadata
{
    internal class DbEntityMetadataManager : IDbEntityMetadataManager
    {
        ...
        //public virtual IDbEntityMetadata AddFromXml(string xmlDefinitionFileContent, string module = null)
        //{
        //    XmlDocument xdoc = new XmlDocument();
        //    xdoc.LoadXml(xmlDefinitionFileContent);

        //    int version = int.Parse(xdoc["version"].Value);
        //    if (version != 1)
        //        throw new NotSupportedException($"Schema version '{version}' is not supported");


        //    string schemaName = xdoc["schemaName"].Value;
        //    string name = xdoc["name"].Value;
        //    string primaryKeyFieldName = xdoc["primaryKey"].Value;
        //    if (module.IsNullOrEmpty())
        //        module = xdoc["module"]?.Value;
        //    string prefix = xdoc["dbPrefix"]?.Value ?? xdoc["prefix"]?.Value;
        ...
        //    XmlNode fieldsNode = xdoc["fields"];
        //    foreach (XmlNode fieldNode in fieldsNode.ChildNodes)
        //    {
        //        IDbFieldMetadata fm = this.AddXmlField(em, (XmlElement)fieldNode);
        //        em.AddFieldIfNotExist(fm);
        //    }

        //    this.Check(em);

        //    this.metadataIndex.Set(name, em);
