...
internal class PostgreSqlSequence : IIntSequence
{
    protected PostgreSqlServerDataModificationProvider Parent { get; }
    protected PostgreSqlDdlGenerator DdlGenerator { get; }
    ...
    public PostgreSqlSequence(PostgreSqlServerDataModificationProvider parent, string name)
    {
        this.Parent = parent;
        ...
        this.DdlGenerator = new PostgreSqlDdlGenerator();
    }
    ...
    public async Task CreateIfNotExists(long minValue, long currentValue)
    {
        if (minValue < 0)
            minValue = 1;
